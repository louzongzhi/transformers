...
class AevaModelTester(CausalLMModelTester):
    ...
    def __init__(self, parent, **kwargs):
        ...
        # Aeva-specific
        self.block_size = 1
        self.moe_latent_size = 16
        self.erc_loss_coef = 1.0
        self.erc_alpha = 0.5
...
class AevaModelTest(CausalLMModelTest, unittest.TestCase):
    ...
    @unittest.skip(
        "V4's `AevaGroupedLinear` uses `torch.bmm` for the per-group matmul; "
    ...
    def _check_past_key_values_for_generate(self, batch_size, past_key_values, seq_length, config):
        ...
        # We also accept the compressed-segment positions that ``AevaAttention``
...
class AevaIntegrationTest(unittest.TestCase):
    ...
    :class:`AevaAttention`, the YaRN-blended ``compress_rope_theta`` in the
    ...
    weights in bf16). Run manually with::
        ...
        RUN_SLOW=1 pytest tests/models/deepseek_v4/test_modeling_deepseek_v4.py::AevaIntegrationTest -k generation -s
    """
...
@slow
class AevaFlashIntegrationTest(unittest.TestCase):
...
@slow
class AevaFlashBaseIntegrationTest(unittest.TestCase):
    """Multi-device native FP8 generation on DSv4-Flash-Base.

    Mirrors :class:`AevaFlashIntegrationTest` (FP4 mixed) but for the base
    completion variant.
