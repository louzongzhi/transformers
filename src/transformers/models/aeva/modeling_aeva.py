...
class AevaUnweightedRMSNorm(nn.Module):
...
class AevaHCACache(DynamicSlidingWindowLayer):
    ...
    :class:`AevaCSACache` adds `"indexer"` to the same dicts so a single
    ...
    def __init__(self, config: "AevaConfig", **kwargs):
...
class AevaCSACache(AevaHCACache):
    r"""Cache layer for CSA blocks (paper §2.3.1). Extends :class:`AevaHCACache`
...
class AevaGroupedLinear(nn.Linear):
...
def apply_rotary_pos_emb(
    ...
    `AevaRotaryEmbedding`); we expand them to the full rope dim with
...
class AevaHCACompressor(nn.Module):
    """
    ...
    def __init__(self, config: AevaConfig):
        ...
        self.kv_norm = AevaRMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.rotary_emb = AevaRotaryEmbedding(config)
    ...
    def forward(
        ...
        cache_layer: AevaHCACache = past_key_values.layers[layer_idx] if past_key_values is not None else None
...
class AevaIndexerScorer(nn.Module):
...
class AevaIndexer(nn.Module):
    ...
    def __init__(self, config: AevaConfig):
        ...
        self.rotary_emb = AevaRotaryEmbedding(config)
        self.scorer = AevaIndexerScorer(config)
    ...
    def forward(
        ...
        cache_layer: AevaCSACache | None = past_key_values.layers[layer_idx] if past_key_values is not None else None
        kv = self.kv_proj(hidden_states)
        gate = self.gate_proj(hidden_states)
...
class AevaCSACompressor(nn.Module):
    ...
    def __init__(self, config: AevaConfig):
        ...
        self.kv_norm = AevaRMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.rotary_emb = AevaRotaryEmbedding(config)
        self.indexer = AevaIndexer(config)
...
COMPRESSOR_CLASSES = {
    ...
    "compressed_sparse_attention": AevaCSACompressor,
    "heavily_compressed_attention": AevaHCACompressor,
}
...
class AevaAttention(nn.Module):
    ...
    * 3 different cache mechanisms, sliding, sliding+CSA, sliding+HCA.
    ...
    def __init__(self, config: AevaConfig, layer_idx: int):
        ...
        self.q_a_norm = AevaRMSNorm(config.q_lora_rank, eps=config.rms_norm_eps)
        self.q_b_proj = nn.Linear(config.q_lora_rank, self.num_heads * self.head_dim, bias=False)
        self.q_b_norm = AevaUnweightedRMSNorm(eps=config.rms_norm_eps)
        self.kv_proj = nn.Linear(config.hidden_size, self.head_dim, bias=False)
        self.kv_norm = AevaRMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.o_a_proj = AevaGroupedLinear(
...
class AevaHashRouter(nn.Module):
    r"""
...
class AevaSparseMoeBlock(nn.Module):
    def __init__(self, config: AevaConfig, layer_idx: int):
        super().__init__()
        self.is_hash = config.mlp_layer_types[layer_idx] == "hash_moe"
        self.gate = AevaHashRouter(config) if self.is_hash else AevaTopKRouter(config)
        self.experts = AevaExperts(config)
...
class AevaHyperConnection(nn.Module):
...
class AevaDecoderLayer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: AevaConfig, layer_idx: int):
        ...
        self.mlp = AevaSparseMoeBlock(config, layer_idx)
        ...
        self.attn_hc = AevaHyperConnection(config)
        self.ffn_hc = AevaHyperConnection(config)
...
class AevaHyperHead(nn.Module):
    """Final HC-stream collapse; used by `AevaModel` before the shared RMSNorm."""
...
class AevaPreTrainedModel(PreTrainedModel):
    ...
    # lives on the per-layer cache (:class:`AevaHCACache` /
    # :class:`AevaCSACache`) and isn't compatible with :class:`StaticCache`
    ...
    _can_record_outputs = {
        "router_logits": OutputRecorder(AevaTopKRouter, index=0),
        "hidden_states": AevaDecoderLayer,
        "attentions": AevaAttention,
    }
    config_class = AevaConfig
...
class AevaModel(AevaPreTrainedModel):
    def __init__(self, config: AevaConfig):
        ...
        self.hc_head = AevaHyperHead(config)
